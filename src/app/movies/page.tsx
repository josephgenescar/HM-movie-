...
import { demoMovies } from "@/lib/demo-content";
import { listPublishedContent } from "@/features/content/content-service";

export default async function MoviesPage() {
  const publishedMovies = await listPublishedContent("movie");
  const movies = publishedMovies.length > 0 ? publishedMovies.map((item) => ({
    title: item.title,
    meta: [item.year, item.duration].filter(Boolean).join(" · "),
    genre: item.genre,
    image: item.posterUrl || demoMovies[0]!.image,
  ...
  return (
    <main className="min-h-screen bg-hm-bg px-5 py-6 sm:px-8 lg:px-10">
      ...
      <section className="mx-auto max-w-7xl pb-20 pt-20">
        <p className="text-xs font-semibold uppercase tracking-[0.2em] text-hm-gold">Bibliothèque</p>
        <div className="mt-3 flex flex-wrap items-end justify-between gap-4 border-b border-hm-border pb-6">
          <div><h1 className="font-display text-4xl font-bold tracking-tight text-hm-text sm:text-5xl">Films</h1><p className="mt-3 max-w-lg text-sm leading-6 text-hm-muted">Des histoires à découvrir, des grands classiques aux nouveautés qui font parler.</p></div>
        ...
        <div className="mt-10"><MediaGrid items={movies} /></div>
      </section>
    </main>
  );
}
